...
if( $conn == false )
    ...
    die (' Error connecting to the SQL Server database.'); //если не получилось подсоединиться
    }
...
if ($result_users_sel_1 > 0){ //сначала проверяем, есть ли уже такой емеил
    while($row = sqlsrv_fetch_array ($result_users_sel_1)) {
        ...
        echo '<script>setTimeout(function(){document.location.href = "/index.html"}, 3500)</script>'; //если такой емеил есть - выходим
    }
    }// else{echo ' 1 if не сработал';} -- для отладки

if (!sqlsrv_has_rows($result_users_sel_1)){ //если такого емеила нет, проверяем логин

    $query_users_sel_2 = "SELECT registered FROM dbo.users WHERE login = '$user_login'";
    ...
    if (!sqlsrv_has_rows($result_users_sel_2)){ //если ни емеила, ни логина нет - инсертим в БД введённые данные(создаём юзера)
        ...
        $result_users_ins = sqlsrv_query($conn, $query_users_ins)
            or die(' Error querying a MSSQL database INSERTing user data. <br/> Please return to the page and try again.'); //на случай, если ошибся с командой/логарифмом и т.д.
        ...
        // else  {echo ' 4 if не сработал';}  -- для отладки
